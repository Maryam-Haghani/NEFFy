...
\item {\bfseries{Paired MSA and Unpaired MSA NEFF}}\+: The tool can recognize paired MSA sequences and unpaired sequences across chains in a block-\/diagonal fashion, as used by Alpha\+Fold-\/\+Multimer. By providing the length of the first monomer, the tool can compute NEFF for each set.
...
For further details, please refer to the \doxylink{neff_usage}{Usage} section and \doxylink{neff_example}{Example}.

\label{neff_usage}%
\Hypertarget{neff_usage}%
\hypertarget{neff_autotoc_md22}{}\doxysubsection{\texorpdfstring{Usage}{Usage}}\label{neff_autotoc_md22}
...
To calculate NEFF, provide an MSA file and indicate the desired flags. NEFF will be computed and presented as the result.\hypertarget{neff_autotoc_md23}{}\doxysubsubsection{\texorpdfstring{Options\+:}{Options\+:}}\label{neff_autotoc_md23}
...
{\ttfamily -\/-\/first\+\_\+monomer\+\_\+length=\texorpdfstring{$<$}{<}value\texorpdfstring{$>$}{>}}   &Length of the first monomer, which is used to obtain NEFF for both paired MSA and individual monomer MSA   &when {\itshape multimer\+\_\+\+MSA}=true   &0   &{\ttfamily -\/-\/first\+\_\+monomer\+\_\+length=17}    \\\cline{1-5}
{\ttfamily -\/-\/column\+\_\+neff=\mbox{[}true/false\mbox{]}}   &Compute Column-\/wise NEFF   &No   &false   &{\ttfamily -\/-\/column\+\_\+neff=true}   \\\cline{1-5}
\end{longtabu}
\hypertarget{neff_autotoc_md24}{}\doxysubsubsection{\texorpdfstring{Supported File Formats}{Supported File Formats}}\label{neff_autotoc_md24}
...
\label{neff_example}%
\Hypertarget{neff_example}%
\hypertarget{neff_autotoc_md25}{}\doxysubsection{\texorpdfstring{Example}{Example}}\label{neff_autotoc_md25}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{./neff\ -\/-\/file=example.sto\ -\/-\/multimer\_MSA=true\ -\/-\/first\_monomer\_length=20}

\end{DoxyCode}
 The tool will identify paired MSA sequences and sequences for the individual MSAs of monomers in the {\itshape example.\+sto} file, assuming the first monomer has a length of 20 residues. It will report NEFF values for the paired MSA as well as for the MSAs corresponding to unpaired sequences. If the provided MSA is not in the form of a multimer MSA, the tool will raise an error stating "{}\+MSA is not in the form of multimer MSA"{}.
\end{DoxyItemize}
\begin{DoxyItemize}
